...
import { IBook, IBookingPayload, IDBPayment, IPayment } from '../../../globals/interface/book';
...
export class ProfileComponent implements OnInit {
  ...
  constructor(
    ...
    this.securityForm.addValidators(
      this.mustMatch(this.securityForm.get('new_password'), this.securityForm.get('confirm_password'))
    )
  ...
  async updateUser(): Promise<void> {
    if(this.profileForm.invalid) {
      ...
      try {
        const response = await this.http_user.updateUser(this.profileForm.value);
  ...
  async updatePassword(): Promise<void> {
    try {
      ...
      const response = await this.http_user.updatePassword(this.securityForm.value);
      this.snackBar._showSnack(response.message, "success");
      alert("System logout...");
  ...
  async getBook() {
    try {
      ...
      const result = data.filter((x) => (x.booker === this.user.id));
